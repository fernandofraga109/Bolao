...
import { Trophy, ArrowRight, Lock, Mail, User as UserIcon, Hash, LogIn, UserPlus } from 'lucide-react';
import { User, Group } from '../types';

interface LoginProps {
  onLogin: (user: User) => void;
  availableUsers: User[];
  onRegister: (name: string, email: string, pass: string, code: string) => { success: boolean, message?: string };
  onAuth: (email: string, pass: string) => { success: boolean, message?: string, user?: User };
}

type AuthMode = 'LOGIN' | 'REGISTER';

const Login: React.FC<LoginProps> = ({ onLogin, availableUsers, onRegister, onAuth }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
      ...
      setTimeout(() => {
          const result = onAuth(loginEmail, loginPass);
  ...
  const handleRegister = (e: React.FormEvent) => {
      ...
      if (!regCode.trim() || !regName.trim() || !regEmail.trim() || !regPass.trim()) {
      ...
      setLoading(true);
      setTimeout(() => {
          const result = onRegister(regName, regEmail, regPass, regCode);
          if (result.success) {
    ...
    <div className="min-h-screen bg-brand-dark flex flex-col items-center justify-center p-4">
      <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md border border-slate-700 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-brand-green/20 to-brand-blue/20 p-8 text-center border-b border-slate-700">
            ...
            <h1 className="text-2xl font-bold text-white mb-1">Bolão Copa 2026</h1>
            <p className="text-slate-400 text-xs uppercase tracking-wider font-semibold">Faça sua aposta</p>
        </div>
        ...
        <div className="p-8">
            ...
            {mode === 'LOGIN' ? (
                ...
                <form onSubmit={handleRegister} className="space-y-4 animate-fadeIn">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-400 uppercase ml-1">Código do Grupo</label>
                        <div className="relative">
                            <Hash className="absolute left-3 top-3 text-indigo-400" size={18} />
                            <input 
                                type="text" 
                                required
                                value={regCode}
                    ...
                    >
                        {loading ? 'Criando Conta...' : 'Cadastrar e Entrar'} <ArrowRight size={18} />
                    </button>
                </form>
            )}
        </div>

        {/* Developer Quick Login (Optional, keep for testing) */}
        <div className="border-t border-slate-700 p-4 bg-slate-900/50">
          <p className="text-[10px] text-slate-500 mb-2 uppercase tracking-wide text-center">Acesso Rápido (Desenvolvimento)</p>
