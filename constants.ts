...
export const OFFICIAL_TOURNAMENT_RESULTS: TournamentPredictions | undefined =
  undefined;

// Export Groups directly from DB file
export const INITIAL_GROUPS: Group[] = DB_GROUPS.map((g) => ({
  ...g,
  competitionCode: g.competitionCode || "WC",
}));
...
export const INITIAL_USERS: User[] = DB_USERS.map((user) => {
  ...
  const userGroupRelations = DB_USER_GROUPS.filter(
  ...
  const userTournPred = DB_TOURNAMENT_PREDICTIONS.find(
    (tp) => tp.userId === user.id,
  );
  ...
  return {
    ...
    activeGroupId:
      user.activeGroupId && groupIds.includes(user.activeGroupId)
        ? user.activeGroupId
        : groupIds.length > 0
          ? groupIds[0]
          : undefined,
