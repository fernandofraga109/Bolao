...
- [ ] Expand `preds` query in `recalculateUserGroupPoints` to include `points`
- [ ] Build `predPointsUpdates` list alongside `pointsByUser` 
- [ ] Batch-update `predictions.points` after `user_groups` upsert
- [ ] Remove `upsertPrediction` call from `batchProcessPointsForMatches`
